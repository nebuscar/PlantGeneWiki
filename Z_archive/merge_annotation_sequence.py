    ...
    with open(output_file, 'w') as out:
        ...
        for record in SeqIO.parse(fasta_file, "fasta"):
            ...
            if gene_id in gene_info:
                ...
                out.write(f"{gene_id}\t{info['chromosome']}\t{info['start']}\t{info['end']}\t{info['strand']}\t{species}\t{fasta_format}\n")
                matched_count += 1
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("用法: python merge_annotation_sequence.py <注释文件> <基因组文件> <输出文件> [物种名]")
