...
# 用法：bash scripts/services/plantgenewiki.sh [start|stop|status]
...
start_all() {
    ...
    echo ""
    echo "日志目录: $LOG_DIR"
    echo "停止服务: bash scripts/services/plantgenewiki.sh stop"
    echo ""
...
case "$1" in
    ...
    status)
        ...
        ;;
    *)
               echo "用法: bash scripts/services/plantgenewiki.sh {start|stop|status}"
        echo ""
        echo "  start   启动所有服务"
        echo "  stop    停止所有服务"
        echo "  status  查看服务状态"
